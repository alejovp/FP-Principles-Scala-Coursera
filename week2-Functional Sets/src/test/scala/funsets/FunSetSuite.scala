...
class FunSetSuite extends FunSuite {
  ...
  test("intersect returns all common elements between two sets") {
    new TestSets {
      val s = union(s1, s2)
  ...
  test("diff returns the set of all elements of `s` that are not in `t`") {
    new TestSets {
      ...
      assert(!contains(sd, 2), "Difference 2")
  ...
  test("forall returns whether all bounded integers within `s` satisfy `p`") {
    new TestSets {
      val s = union(s1, s2)
      val p1 = union (s1, s3)
      val p2 = union(union(s1, s2), s3)
      assert(!forall(s, p1), "Forall 1")
      assert(forall(s, p2), "Forall 2")
      assert(!forall(s, s1), "Forall 3")
      assert(forall(s1, s1), "Forall 4")
    }
  }
  
  test("exists returns whether there exists a bounded int within `s` that satisfies `p`") {
    new TestSets {
      val s = union(s1, s2)
      val p1 = union (s1, s3)
      val p2 = union(union(s1, s2), s3)
      ...
      assert(exists(s, s1), "Exists 3")
      assert(!exists(s, s3), "Exists 4")
    }
  }
